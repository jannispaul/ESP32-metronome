...
static Buttons s_buttons{BUTTON_DEBOUNCE_MS};                // Entprellen wie vorher
...
static void logic_task(void*) {
    Serial.println("\xF0\x9F\xA4\xA0 LogicTask running (Core 0)");
    s_buttons.begin();

    for(;;) {
        // 1) Button-Events lesen (entprellt)
        ButtonEvents ev = s_buttons.poll();
        if (ev.nextPressed) {
            Serial.println("\xF0\x9F\x9F\xA2 NEXT gedrückt");
            onNextRequested();
        }
        if (ev.mutePressed) {
            Serial.println("\xF0\x9F\x9F\xA2 MUTE gedrückt");
