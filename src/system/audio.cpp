...
// Wunsch-BPM, die am nächsten Beat committed wird (0 = kein Pending)
static std::atomic<uint16_t> s_bpm_req{0};
...
static void audio_task(void*) {
  ...
  uint16_t req0 = s_bpm_req.load(std::memory_order_acquire);
  if (req0 == 0) req0 = 120;              // failsafe Default
  s_bpm = req0;
  uint32_t period_ms = bpm_to_period_ms(s_bpm);
  s_periodTicks = ms_to_ticks(period_ms);
  s_epochTicks  = xTaskGetTickCount();
  Serial.printf("⏱️ AudioTask ready \n BPM=%u \n period=%u ms\n", (unsigned)s_bpm, (unsigned)period_ms);


for (;;) {
  TickType_t nowTicks = xTaskGetTickCount();

  // Nächste Beat-Grenze unter aktuellem Raster bestimmen
  TickType_t next_old = next_aligned_beat_after(nowTicks);

  // --- Commit eines ausstehenden BPM-Requests exakt an der kommenden Beat-Grenze ---
  uint16_t req = s_bpm_req.load(std::memory_order_acquire);
  if (req != 0 && req != s_bpm) {
    s_bpm = req;
    uint32_t period_ms   = bpm_to_period_ms(s_bpm);
    ...
    // Epoche so setzen, dass der nächste Beat zeitlich unverändert bleibt
    s_periodTicks = newPeriod;
    s_epochTicks  = next_old - s_periodTicks;

#if AUDIO_TELEM
    Serial.printf("⏱️ BPM(commit)=%u | period=%u ms\n",
                  (unsigned)s_bpm, (unsigned)period_ms);
#endif
  }
...
void audio_set_bpm(uint16_t bpm) {
  if (bpm == 0) bpm = 1;
  s_bpm_req.store(bpm, std::memory_order_release);  // nur Wunsch setzen

  // rein informativ – Commit erfolgt an der nächsten Beat-Grenze:
#if AUDIO_TELEM
  Serial.printf("⏱️ BPM(request)=%u | period=%u ms (commit at next beat)\n",
                (unsigned)bpm, (unsigned)bpm_to_period_ms(bpm));
#endif
}
