...
void displayUI() {
  ...
  temp = round((1 / (bpm / 60.0)) * 1000.0);
  char tempoStr[3]; // Buffer to store the tempo value as string
  sprintf(tempoStr, "%d", temp); // Convert tempo to string
  u8g2.drawStr(57, 41, tempoStr); // Draw tempo on the display
